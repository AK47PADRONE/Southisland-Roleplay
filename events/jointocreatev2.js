...
module.exports = function (client) {
  ...
  client.on("voiceStateUpdate", (oldState, newState) => {
    ...
    if (!oldState.channelID && newState.channelID) {
      if (newState.channelID !== config.JOINTOCREATECHANNELV2) return;  //if its not the jointocreatechannel skip
    ...
    if (oldState.channelID && newState.channelID) {
      ...
      if (oldState.channelID !== newState.channelID) {
        ...
        if (newState.channelID === config.JOINTOCREATECHANNELV2)
          //make a new channel
  ...
  async function jointocreatechannel(user) {
    ...
    await user.guild.channels.create(`💸Donate`, {
      type: 'voice',
